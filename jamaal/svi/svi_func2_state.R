...
#' @param state The state, if one is isolated, for the SVI
#'
#' @return A simple features object with the basic demographic data.
#'
#' @importFrom tidycensus get_acs
get_basic_demo <- function(geography, census_year, state = NULL) {
  basic_demo <- get_acs(
    geography = geography,
    variables = c(
...
#' @importFrom dplyr %>%
build_rpl1 <- function(geography, census_year, basic_demo, state = NULL) {
  rpl1 <- get_acs(
    geography = geography,
    variables = c(
...
#' @importFrom dplyr percent_rank
#' @importFrom dplyr %>%
build_rpl2 <- function(geography, census_year, basic_demo, state = NULL) {
  rpl2 <- get_acs(
    geography = geography,
    variables = c(
...
#' @importFrom dplyr percent_rank
#' @importFrom dplyr %>%
build_rpl3 <- function(geography, census_year, basic_demo, state = NULL) {
  rpl3 <- get_acs(
    geography = geography,
...
#' @param census_year The year of the ACS data to get.
#' @param basic_demo The basic demographic data.
#' @param state  The state, if one is isolated, for the SVI description
#'
...
#' @importFrom dplyr percent_rank
#' @importFrom dplyr %>%
build_rpl4 <- function(geography, census_year, basic_demo, state = NULL) {
  rpl4 <- get_acs(
    geography = geography,
    variables = c(
...
#' @importFrom dplyr left_join
#' @importFrom dplyr %>%
build_svi_state <- function(geography, census_year, state = NULL) {
  ...
  basic_demo <- get_basic_demo(geography, census_year, state)
  
  rpl1 <- build_rpl1(geography, census_year, basic_demo, state)
  rpl2 <- build_rpl2(geography, census_year, basic_demo, state)
  rpl3 <- build_rpl3(geography, census_year, basic_demo, state)
  rpl4 <- build_rpl4(geography, census_year, basic_demo, state)
